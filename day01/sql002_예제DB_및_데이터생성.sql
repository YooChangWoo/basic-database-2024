-- 데이터베이스 생성 / 최초에 새로운 DB 생성할 떄는 master(기본) 에서 생성해야하 함
-- USE는 해당 DB를 사용하겠다라는 뜻
USE [master] --[]는 데이터베이스와 컬럼명에만 사용

-- 마당 데이터베이스 생성
     DROP DATABASE [Madang]; -- C++ 동일하게 한 문장 끝은 ; (생략가능)
CREATE DATABASE Madang
GO -- 여기까지 일단 한번 실행

-- GO 명령어는 T-SQL 문은 아니고, 현재까지 명령어를 SQL Server에 보내서 실행시키는 역할을 한다. 
