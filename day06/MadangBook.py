...
class qtApp(QMainWindow):
    ...
    def initUI(self) -> None:
        # 입력제한
        self.txtBookId.setValidator(QIntValidator(self)) # 숫자만 입력하도록 제안
        self.txtPrice.setValidator(QIntValidator(self)) # 숫자만 입력하도록 제안
    ...
    def btnSaveClicked(self): # 저장버튼 
        ...
        # 2. 현재 존재하는 번호를 사용했는지 체크, 이미 있는 번호면 DB입력 쿼리 실행이 안되도록 막아야 함
            ...
            query = f'''SELECT COUNT(*)
                          FROM Book
                         WHERE bookid = {bookid}''' # 현재 입력하고자하는 번하ㅗ가 있는 확인 쿼리
    ...
    def btnDelClicked(self):  # 삭제버튼 
        ...
        # Validation Check
        if bookid == None or bookid == '':
            QMessageBox.warning(self, '삭제경고', '책 번호 없이 삭제할 수 없습니다. 똑바로 해라 인간아')
            return
