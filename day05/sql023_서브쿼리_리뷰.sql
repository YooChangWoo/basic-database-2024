-- 서브쿼리 리뷰
-- WHERE 절 서브쿼리
-- ALL, ANY(SOME) - ANY(SOME) 쓰는 조건은 아래의 데이터를 사용어렵다
...
-- SELECT절 서브쿼리, JOIN으로 변경가능 이미 쿼리가 너무 복잡해서 더이상 테이블 추가하기 힘들때 많이 사용
...
SELECT SUM(o.saleprice) AS '고객별 판매액'
 ...
 -- UPDATE, 한꺼번에 필요한 필드값을 한테이블에서 다른테이블로 복사할때 아주 유용함
 UPDATE Orders
...
-- 고객별 판매액으로 고객명과 판매금액을 보이시오(서프쿼리 --> 조인)
