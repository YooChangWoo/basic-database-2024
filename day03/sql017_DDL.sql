...
/*
    ...
    CHAR(n)[고정문자], VARCHAR(n)[가변문자열], NCHAR(n), NVARCHAR(n), N- [유니코드],
    ...
    IMAGE[이미지바이너리], SALLINT[255까지의 정수], TEXT[2GB까지의 텍스트], NTEXT[유니코드 2G]
...
CREATE TABLE NewBook(
    ...
    price INT DEFAULT 10000 CHECK(PRICE > 1000)  -- 기본값 제약조건, 체크 제약조절
    PRIMARY KEY (bookname, publisher) -- 개체ㅣ 무결정 제약조건
);
