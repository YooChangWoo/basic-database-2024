...
DECLARE @i INT;
 ...
 -- 인덱스가 없는 상태
 -- 100만건 정도 정도 데이터 조회시 5~8초 사이 시간 소요
 ...
 -- ! username을 WHERE에 검색을 위해서 username을 사용함
 ...
 -- WHERE절, JOIN의 ON 절에 들어가는 컬럼에 인덱스를 설정하는 것에 속도 개선에 도움
 -- 1번 PK, 2번 FK, 3번 WHERE절에 검색시 들어가는 컬럼에 인덱스 설정
